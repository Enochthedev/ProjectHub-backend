import {
  IsString,
  IsEnum,
  IsInt,
  IsArray,
  IsBoolean,
  IsOptional,
  IsUrl,
  MinLength,
  MaxLength,
  Min,
  Max,
  ArrayMaxSize,
  IsIn,
} from 'class-validator';
import { Transform, Type } from 'class-transformer';
import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';
import { DifficultyLevel } from '../../common/enums/difficulty-level.enum';
import { SPECIALIZATIONS } from '../../common/constants/specializations';

export class CreateProjectDto {
  @ApiProperty({
    description: 'Project title - should be descriptive and concise',
    example: 'AI-Powered Student Performance Analytics Dashboard',
    minLength: 10,
    maxLength: 200,
    type: String,
  })
  @IsString({ message: 'Title must be a string' })
  @MinLength(10, { message: 'Title must be at least 10 characters long' })
  @MaxLength(200, { message: 'Title must not exceed 200 characters' })
  @Transform(({ value }) => (typeof value === 'string' ? value.trim() : value))
  title: string;

  @ApiProperty({
    description:
      'Detailed project abstract describing objectives, methodology, and expected outcomes',
    example:
      'This project aims to develop an intelligent dashboard that analyzes student performance data using machine learning algorithms. The system will provide insights into learning patterns, identify at-risk students, and suggest personalized interventions.',
    minLength: 100,
    maxLength: 2000,
    type: String,
  })
  @IsString({ message: 'Abstract must be a string' })
  @MinLength(100, { message: 'Abstract must be at least 100 characters long' })
  @MaxLength(2000, { message: 'Abstract must not exceed 2000 characters' })
  @Transform(({ value }) => (typeof value === 'string' ? value.trim() : value))
  abstract: string;

  @ApiProperty({
    description: 'Project specialization area',
    example: 'Artificial Intelligence & Machine Learning',
    enum: SPECIALIZATIONS,
    type: String,
  })
  @IsString({ message: 'Specialization must be a string' })
  @IsIn(SPECIALIZATIONS, {
    message: `Specialization must be one of: ${SPECIALIZATIONS.join(', ')}`,
  })
  specialization: string;

  @ApiProperty({
    description:
      'Project difficulty level indicating complexity and required skills',
    example: DifficultyLevel.INTERMEDIATE,
    enum: DifficultyLevel,
  })
  @IsEnum(DifficultyLevel, {
    message: `Difficulty level must be one of: ${Object.values(DifficultyLevel).join(', ')}`,
  })
  difficultyLevel: DifficultyLevel;

  @ApiProperty({
    description: 'Academic year when the project was completed',
    example: 2024,
    minimum: 2020,
    maximum: new Date().getFullYear(),
    type: Number,
  })
  @Type(() => Number)
  @IsInt({ message: 'Year must be an integer' })
  @Min(2020, { message: 'Year must be at least 2020' })
  @Max(new Date().getFullYear(), { message: 'Year cannot be in the future' })
  year: number;

  @ApiProperty({
    description: 'Project tags for categorization and search optimization',
    example: [
      'machine-learning',
      'dashboard',
      'analytics',
      'education',
      'python',
    ],
    type: [String],
    maxItems: 10,
  })
  @IsArray({ message: 'Tags must be an array' })
  @IsString({ each: true, message: 'Each tag must be a string' })
  @ArrayMaxSize(10, { message: 'Maximum 10 tags allowed' })
  @Transform(({ value }) =>
    Array.isArray(value)
      ? value
          .map((tag: any) => (typeof tag === 'string' ? tag.trim() : ''))
          .filter(Boolean)
      : [],
  )
  tags: string[];

  @ApiProperty({
    description: 'Technologies, frameworks, and tools used in the project',
    example: [
      'Python',
      'TensorFlow',
      'React',
      'Node.js',
      'PostgreSQL',
      'Docker',
    ],
    type: [String],
    maxItems: 15,
  })
  @IsArray({ message: 'Technology stack must be an array' })
  @IsString({ each: true, message: 'Each technology must be a string' })
  @ArrayMaxSize(15, { message: 'Maximum 15 technologies allowed' })
  @Transform(({ value }) =>
    Array.isArray(value)
      ? value
          .map((tech: any) => (typeof tech === 'string' ? tech.trim() : ''))
          .filter(Boolean)
      : [],
  )
  technologyStack: string[];

  @ApiProperty({
    description:
      'Whether this project is suitable for group work or individual work',
    example: false,
    type: Boolean,
  })
  @Type(() => Boolean)
  @IsBoolean({ message: 'isGroupProject must be a boolean' })
  isGroupProject: boolean;

  @ApiPropertyOptional({
    description: 'GitHub repository URL for the project source code',
    example: 'https://github.com/username/student-analytics-dashboard',
    format: 'uri',
    maxLength: 500,
    type: String,
  })
  @IsOptional()
  @IsUrl({}, { message: 'GitHub URL must be a valid URL' })
  @MaxLength(500, { message: 'GitHub URL must not exceed 500 characters' })
  githubUrl?: string;

  @ApiPropertyOptional({
    description: 'Live demo or deployment URL for the project',
    example: 'https://student-analytics.example.com',
    format: 'uri',
    maxLength: 500,
    type: String,
  })
  @IsOptional()
  @IsUrl({}, { message: 'Demo URL must be a valid URL' })
  @MaxLength(500, { message: 'Demo URL must not exceed 500 characters' })
  demoUrl?: string;

  @ApiPropertyOptional({
    description:
      'Additional notes, requirements, or special instructions for the project',
    example:
      'This project requires access to student data and should comply with GDPR regulations. Recommended for students with prior experience in data science.',
    maxLength: 1000,
    type: String,
  })
  @IsOptional()
  @IsString({ message: 'Notes must be a string' })
  @MaxLength(1000, { message: 'Notes must not exceed 1000 characters' })
  @Transform(({ value }) => (typeof value === 'string' ? value.trim() : value))
  notes?: string;
}
